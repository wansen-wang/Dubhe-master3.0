...
/**
 * @description 难例发布中状态类
 * @date 2022-05-07
 **/
@Component
public class PcDatasetDifficultCasePublishingState extends AbstractPcDatasetState {
    @Autowired
    ...
    public void difficultCaseFailedToPublishEvent(Long datasetId, String statusDetail) {
        pcDatasetService.updatePcDataset(new LambdaUpdateWrapper<PcDataset>()
                .eq(PcDataset::getId, datasetId)
                .set(PcDataset::getStatus, PcDatasetMachineStatusEnum.DIFFICULT_CASE_FAILED_TO_PUBLISH.getCode())
                .set(PcDataset::getStatusDetail, statusDetail));
    }

    @Override
    public String currentStatus() {
        return PcDatasetMachineStatusEnum.DIFFICULT_CASE_PUBLISHING.getDesc();
    }
}
