...
class Detector(object):
    ...
    def de_coco_format(self, ann_json):
        ...
        if ann_json:
            for i, annotation in enumerate(ann_json):
                ...
                cls_ = int(annotation['category_id'])
    ...
    #跟踪处理，用于自动标定
    def run_track(self, image_list, label_list, labels):
        frame_no = 0
        avg_fps = 0.0
        self.class_ = labels
        self.kc_tracker = {}
        for cls_id in self.class_:
            kc_tracker = KCTracker(
                ...
                cls_=cls_id)
            self.kc_tracker[cls_id] = kc_tracker
