...
model_load_dir = current_dir + os.sep + "model" + os.sep + "yolov3_model_python/"
        ...
        if result.shape[0] == 0:
            ...
            for j in range(result.shape[0]):
                ...
                if cls_id in label_list:
                    label_name = label_2_name[int(cls_id)]
                    temp['annotation'].append({
                        'area': width * height,
                        'bbox': [x1, y1, width, height],
                        'category_id': label_name.rstrip('\n'),
                        'iscrowd': 0,
