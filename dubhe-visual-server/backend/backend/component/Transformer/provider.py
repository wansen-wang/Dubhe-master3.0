...
def get_transformer_text_data(request):
    ...
    file_path = get_file_path(uid, run, 'transformer', tag)
    data = transformer_text_data_provider(file_path, tag)
    return {tag: data}


def get_transformer_image_data(request):
    params = ['uid', 'trainJobName', 'run', 'tag', 'l', 'x', 'y', 'g', 'r']
    uid, trainJobName, run, tag, l, x, y, g, r = get_api_params(request, params)
    attn_file_path = get_file_path(uid, run, 'transformer', tag + f'-am')
    img_file_path = get_file_path(uid, run, 'transformer', tag)
